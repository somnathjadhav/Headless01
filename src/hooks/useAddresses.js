    ...
    try {
      ...
      if (data.success) {
        ...
        if (addresses.length > 0) {
          ...
          // If no addresses from backend, check localStorage
          const savedAddresses = localStorage.getItem('userAddresses');
          if (savedAddresses) {
            try {
              const parsedAddresses = JSON.parse(savedAddresses);
              console.log('Using saved addresses from localStorage:', parsedAddresses);
      ...
      if (savedAddresses) {
        try {
          const parsedAddresses = JSON.parse(savedAddresses);
          console.log('Using fallback addresses from localStorage:', parsedAddresses);
          setAddresses(parsedAddresses);
        } catch (parseError) {
          console.error('Error parsing saved addresses:', parseError);
        }
